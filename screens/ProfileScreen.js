...
const ProfileScreen = ({ navigation }) => {
  ...
  const [walletLoading, setWalletLoading] = useState(false); // New loading state for wallet
    ...
    try {
      ...
      if (storedUserData) {
        ...
        if (!response.ok) {
          ...
          console.error(
            "Failed to fetch followers data.",
            errorText,
            response.status
          );
          Alert.alert(
            "Lỗi",
            `Không thể lấy danh sách người theo dõi. Lỗi: ${response.status}`
          );
      ...
      Alert.alert("Lỗi", `Lỗi khi tải thông tin người dùng: ${error.message}`);
    }
  }, []);
  ...
  const fetchUserRank = useCallback(async () => {
    try {
      ...
      Alert.alert("Lỗi", "Lỗi khi tải thông tin xếp hạng.");
    }
  }, []);

  const fetchWalletBalance = useCallback(async () => {
    setWalletLoading(true); // Set loading state for wallet
    try {
      ...
      const response = await fetchWithAuth(
        `${API_BASE_URL}/api/v1/wallet?userId=${userId}`
      );

      if (!response.ok) {
        const errorMessage = `Failed to fetch wallet data: ${response.status}`;
      ...
      Alert.alert("Lỗi", "Lỗi khi tải thông tin ví");
    } finally {
      setWalletLoading(false);
    }
  }, []);
  ...
  const fetchUserPosts = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          "Failed to fetch user articles data:",
          response.status,
          errorText
        );
        Alert.alert("Lỗi", `Không thể tải bài viết. Lỗi: ${response.status}`);
        throw new Error(
          `Failed to fetch user articles data: ${response.status}`
        );
      }
      ...
      Alert.alert("Lỗi", "Lỗi khi tải bài viết.");
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (!post.liked) {
        ...
        if (!response.ok) {
          ...
          Alert.alert("Lỗi", "Lỗi khi thích bài viết");
        } else {
          fetchUserPosts();
        }
      } else {
        ...
        if (!response.ok) {
          ...
          Alert.alert("Lỗi", "Lỗi khi bỏ thích bài viết");
        } else {
          fetchUserPosts();
        }
      }
      ...
      Alert.alert("Lỗi", "Lỗi khi xử lý nút thích/bỏ thích bài viết");
    }
  };
